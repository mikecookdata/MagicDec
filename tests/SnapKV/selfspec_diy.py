...
parser.add_argument('--model', type=Path, default=Path("./checkpoints/meta-llama/Llama-3.2-1B/model.pth"), help='model')
...
parser.add_argument('--draft_budget', type=int, default=257, help='Dataset end index.')
...
parser.add_argument('--max_len', type=int, default=16128, help='Generate length')
...
num_eval_steps = min(1, len(dataloader))
...
for step, batch in tqdm(enumerate(dataloader), total=num_eval_steps):
    ...
    while terminal == False: # stop speculation when length exceeds MAX_LEN_TARGET
    ...
    # Verification
        ...
        print(f'accept_nums: {accept_nums}')
