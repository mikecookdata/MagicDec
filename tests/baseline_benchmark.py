...
global_group = None
if use_tp: # tensor parallism, multiple gpus
...
engine.load_model(checkpoint_path, use_tp=use_tp, rank_group = args.rank_group, group=global_group) # what happend behind this
if args.compile:
   engine.compile()
...
for step, batch in tqdm(enumerate(dataloader), total=num_eval_steps):
   ...
   while output.size(1)<MAX_LEN and terminate == False:
       ...
       print(f'next_tokens: {next_tokens}') # from print, all just contain one token
       output = torch.cat((output, next_tokens),dim=-1)
       model_steps += 1
   ...
   if args.printoutput:
       for i in range(BATCH_SIZE):
           ...
           # print("---++++--")
   print(f"Tokens per second :{BATCH_SIZE*(model_steps/total_time)}") # why useing steps to count token? this means one step only generate one token?
   if step < 5:
       total_time = 0.0
       model_steps = 0
   if use_tp:
